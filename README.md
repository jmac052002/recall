...
└── tests/                   # Unit tests for config, embeddings, and both memory stores
``` 
...
The test suite mocks all external services (Postgres, Voyage AI) so it runs
anywhere, fast, with no Docker or API keys required:
...
- [x] **Phase 3 — Memory retrieval**: vector + keyword dual search (tested)
